...
$(function(){
    ...
    $("#sheet").on("click",".login-in", function() {
        layer.prompt(
        {
            ...
            closeBtn: false,
            // value: '',  // 默认值
            btn: ['<span class="iconfont icon-edit" style="font-size:.28rem"></span> 确定', '<span class="iconfont icon-roundclose" style="font-size:.28rem"></span> 取消', '<span class="iconfont icon-profile" style="font-size:.28rem"></span> 帮助'],
            btn3: function(index, layero){
                layer.open({
                    title: '如何获取您的网易云UID？'
                    ,closeBtn: false
                    ,btn: ['<span class="iconfont icon-roundclose" style="font-size:.28rem"></span> 知道啦']
                    ,area: '280px;'
                    ,shade: 0.6 //遮罩透明度
                    ,anim: -1 //0-6的动画形式，-1不开启
                    ,content: 
                    '1、首先<a href="http://music.163.com/" target="_blank" class="js-leeleo-color-cur">点我(http://music.163.com/)</a>打开网易云音乐官网<br>' +
                    '2、然后点击页面右上角的“登录”，登录您的账号<br>' + 
                    '3、点击您的头像，进入个人中心<br>' + 
                    '4、此时浏览器地址栏 <span class="js-leeleo-color-cur">/user/home?id=</span> 后面的<span  class="js-leeleo-color-cur">数字</span>就是您的网易云 UID'
                });  
            }
        },
    ...
    // 图片加载失败处理
    // $('img').error(function(){
    //     $(this).attr('src', '/template/JOELEO/asset/inc/leeleo/f-leeleo.jpg');
    // });
...
function musicInfo(list, index) {
    ...
    var tempStr = '<div  class="js-music-color" style="padding: .4rem; line-height: .6rem;font-size: .28rem;"><span class="info-title">歌名：</span>' + music.name + 
    ...
    layer.open({
        ...
        id: 'LAY_layuipro',
        resize: false,
        btn: ['<span class="iconfont icon-roundclose" style="font-size:.28rem"></span> 关闭'],
        btnAlign: 'c',
...
function searchBox() {
    ...
    '        <button class="search-submit" type="submit"><span class="iconfont icon-search"></span></button><button type="submit" style="position: fixed;background: transparent;width: 1.2rem;height: 2rem;outline: 0 none !important;top: 60%;right: 1.2rem;"></button>' + 
    '    </div>' + 
    '    <div class="radio-group" id="music-source">' + 
    '       <label class="leo-label"><input class="leo-radio" type="radio" name="source" value="netease" checked=""><span class="leo-radioInput"></span><img class="leo-img"  src="/template/JOELEO/asset/inc/leeleo/s-netease.png"> </label>' + 
    '       <label class="leo-label"><input class="leo-radio" type="radio" name="source" value="tencent"><span class="leo-radioInput"></span><img class="leo-img"  src="/template/JOELEO/asset/inc/leeleo/s-qq.png"></label>' + 
    '       <label class="leo-label"><input class="leo-radio" type="radio" name="source" value="xiami"><span class="leo-radioInput"></span><img class="leo-img"  src="/template/JOELEO/asset/inc/leeleo/s-xiami.png"></label>' + 
    '       <label class="leo-label"><input class="leo-radio" type="radio" name="source" value="kugou"><span class="leo-radioInput"></span><img class="leo-img"  src="/template/JOELEO/asset/inc/leeleo/s-kugou.png"></label>' + 
    '       <label class="leo-label"><input class="leo-radio" type="radio" name="source" value="baidu"><span class="leo-radioInput"></span><img class="leo-img"  src="/template/JOELEO/asset/inc/leeleo/s-baidu.png"></label>' + 
    '   </div>' + 
    '</div></form>';
    ...
    layer.open({
        ...
        ,area: '280px;'
        ,shade: 0.6 //遮罩透明度
        ,btn: ['<span class="iconfont icon-roundclose" style="font-size:.28rem"></span> 关闭','<span class="iconfont icon-search" style="font-size:.28rem"></span> 搜索']
        ,anim: -1 //0-6的动画形式，-1不开启
        ,content: tmpHtml,
        cancel: function(){
        }
...
function changeCover(music) {
    ...
    if(img == "err") {
        img = "/template/JOELEO/asset/inc/leeleo/f-leeleo.jpg";
    } else {
...
function addSheet(no, name, cover) {
    if(!cover) cover = "/template/JOELEO/asset/inc/leeleo/f-leeleo.jpg";
    if(!name) name = "读取中...";
...
function clearDislist() {
    ...
    if(rem.dislist == 1) {  // 正在播放列表
        ...
        $(".sheet-item[data-no='1'] .sheet-cover").attr('src', '/template/JOELEO/asset/inc/leeleo/f-leeleo.jpg');    // 恢复正在播放的封面
